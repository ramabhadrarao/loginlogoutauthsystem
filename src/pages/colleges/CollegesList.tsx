...
import CollegeFormModal from '../../components/modals/CollegeFormModal';
...
const CollegesList = () => {
  ...
  const handleSubmitCollege = async (collegeData: Partial<College>) => {
    try {
      setModalLoading(true);
      
      if (editingCollege) {
        // Update existing college
        const updatedCollege = await collegesApi.update(editingCollege._id, collegeData);
        setColleges(colleges.map(college => 
          college._id === editingCollege._id ? updatedCollege : college
        ));
      } else {
        // Create new college
        const newCollege = await collegesApi.create(collegeData);
        setColleges([newCollege, ...colleges]);
      }
      
      handleCloseModal();
    } catch (err: any) {
      alert(`Failed to ${editingCollege ? 'update' : 'create'} college: ` + err.message);
